...
def main() -> int:
    ...
    if args.metadata:
        ...
        # ── Centreline CSV export ──────────────────────────────────────────
        # Always written alongside the metadata. Columns:
        #   index, x_mm, y_mm, z_mm, arc_length_mm, radius_mm
        # Useful for: ParaView (CSV Reader → Table To Points), numpy/pandas
        # analysis, computing flow-rate-vs-arc-length plots downstream.
        centreline_csv = os.path.splitext(args.output)[0] + "_centreline.csv"
        with open(centreline_csv, "w", encoding="utf-8") as f:
            f.write("# Centreline points (x,y,z), cumulative arc length, "
                    "and lumen radius at each point.\n")
            f.write("# Generated by blender_aorta_v2 (v3 wrapper passes through "
                    "the v2 generator).\n")
            f.write("index,x_mm,y_mm,z_mm,arc_length_mm,radius_mm\n")
            for i, (p, s_val, r) in enumerate(zip(centres, arc_s, radii)):
                f.write(f"{i},{p.x:.6f},{p.y:.6f},{p.z:.6f},"
                        f"{s_val:.6f},{r:.6f}\n")
